...
public class Mp4Tag extends AbstractTag
{
  ...
  /**
	...
	/**
	 * Delete field of specified index with this key
	 * @param genericKey
	 * @param index
	 * @throws KeyNotFoundException 
	 */
	public void deleteFieldAt(FieldKey genericKey, int index) throws KeyNotFoundException {
		...
		//Use regular deleteField for certain field-key types
		if (genericKey == FieldKey.DISC_NO || genericKey == FieldKey.TRACK ||
			genericKey == FieldKey.DISC_TOTAL || genericKey == FieldKey.TRACK_TOTAL)
		{
			deleteField(genericKey);
		}
		else
		{
			String mp4FieldName = tagFieldToMp4Field.get(genericKey).getFieldName();
			super.deleteFieldAt(mp4FieldName, index);
		}
	...
	/**
     * Delete fields with this mp4key at specified index
     *
     * @param mp4Key
     * @throws org.jaudiotagger.tag.KeyNotFoundException
     */
    public void deleteFieldAt(Mp4FieldKey mp4Key, int index) throws KeyNotFoundException
    {
		//Same as deleteField(Mp4FieldKey)
        if (mp4Key == null)
        {
            throw new KeyNotFoundException();
        }
        super.deleteFieldAt(mp4Key.getFieldName(), index);
    }
