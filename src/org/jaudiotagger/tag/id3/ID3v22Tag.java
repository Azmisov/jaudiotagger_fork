...
public class ID3v22Tag extends AbstractID3v2Tag
{
	...
	/**
	 * ID3v22 doesn't support multiple values; will only delete if index = 0
	 * @param key
	 * @param index
	 */
	public void deleteFieldAt(FieldKey genericKey, int index){
		logger.log(Level.SEVERE, "Unsuported deleteFieldAt format");
		if (index != 1) return;
	 ...
	 * @param index
	 */
	public void deleteFieldAt(ID3v22FieldKey key, int index) throws KeyNotFoundException
	{
		logger.log(Level.SEVERE, "Unsuported deleteFieldAt format");
		if (index != 1) return;
		deleteField(key);
