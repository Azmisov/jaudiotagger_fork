...
public final class AsfTag extends AbstractTag
{
	...
	/**
	 * Remove field stored at provided field key with specified index
	 * @param fieldKey
	 * @param index
	 */
	public void deleteFieldAt(final AsfFieldKey fieldKey, int index) throws KeyNotFoundException
	{
		//Same as deleteField(asffieldkey)
		super.deleteFieldAt(fieldKey.getFieldName(), index);
	}

    @Override
    public void deleteFieldAt(final FieldKey fieldKey, int index) throws KeyNotFoundException
    {
        //Same as deleteField(fieldkey)
		if (fieldKey == null)
        {
            throw new KeyNotFoundException();
        }
        super.deleteFieldAt(tagFieldToAsfField.get(fieldKey).getFieldName(), index);
    }
