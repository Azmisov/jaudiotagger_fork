...
public abstract class AbstractTag implements Tag
{
	 ...
	 * @param index 
	 */
	public void setFieldAt(TagField field, int index){
		if (field == null)
            return;
	...
	/**
	 * @see org.jaudiotagger.tag.Tag#deleteFieldAt(org.jaudiotagger.tag.FieldKey, int) 
	 */
	public abstract void deleteFieldAt(FieldKey fieldKey, int index) throws KeyNotFoundException;
	
	/**
	 * @see org.jaudiotagger.tag.Tag#deleteFieldAt(java.lang.String, int) 
	 */
	public void deleteFieldAt(String key, int index)
	{
		List<TagField> field_list = fields.get(key);
		
		//Return, if the key doesn't exist or index is out of bounds
		if (field_list == null)
			return;
		int l = field_list.size();
		if (index >= l)
			return;
		
		//If this is the last field, just delete the entire key
		if (l == 1) fields.remove(key);
		else field_list.remove(index);
	}
